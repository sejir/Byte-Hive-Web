...
class UpcomingeventsController extends AbstractController
{
    ...
    /**
     * @Route("/newa", name="app_upcomingevents_neww", methods={"GET", "POST"})
     */
    public function neww(Request $request, UpcomingeventsRepository $upcomingeventsRepository): Response
    {
        $upcomingevent = new Upcomingevents();
        $form = $this->createForm(UpcomingeventsType::class, $upcomingevent);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $upcomingeventsRepository->add($upcomingevent);
            return $this->redirectToRoute('app_upcomingeventsapp_upcomingevents_admin', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('upcomingevents_admin/neww.html.twig', [
            'upcomingevent' => $upcomingevent,
            'form' => $form->createView(),
        ]);
    }
    ...
    /**
     * @Route("/{id}/showw", name="app_upcomingevents_showw", methods={"GET"})
     */
    public function showw(Upcomingevents $upcomingevent): Response
    {
        return $this->render('upcomingevents_admin/showw.html.twig', [
            'upcomingevent' => $upcomingevent,
        ]);
    }
    ...
    /**
     * @Route("/{id}/editt", name="app_upcomingevents_editt", methods={"GET", "POST"})
     */
    public function editw(Request $request, Upcomingevents $upcomingevent , UpcomingeventsRepository $upcomingeventsRepository): Response
    {
        $form = $this->createForm(UpcomingeventsType::class, $upcomingevent);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
        ...
        return $this->render('upcomingevents_admin/editw.html.twig', [
            'upcomingevent' => $upcomingevent,
            'form' => $form->createView(),
        ]);
    }
    ...
    /**
     * @Route("/{id}/deletew", name="app_upcomingevents_deletew", methods={"POST"})
     */
    public function deletew(Request $request, Upcomingevents $upcomingevent, UpcomingeventsRepository $upcomingeventsRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$upcomingevent->getId(), $request->request->get('_token'))) {
            $upcomingeventsRepository->remove($upcomingevent);
        }
